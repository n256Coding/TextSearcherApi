...
public final class MongoDBDataModel implements DataModel {
    ...
    private static final Logger log = LoggerFactory.getLogger(com.n256coding.Dev.MongoDBDataModel.class);
...
//            Map<String,Object> user = (Map<String,Object>) cursor.next().toMap();
            Map<String, Object> user = document.entrySet().stream()
...
//        DBObject objectIdLong = collectionMap.find(new BasicDBObject("element_id", id));
        BasicDBObject objectIdLong;
        FindIterable<Document> element_id = collectionMap.find(new BasicDBObject("element_id", id));
        if (element_id != null && element_id.first() != null) {
//            Map<String,Object> idLong = (Map<String,Object>) objectIdLong.toMap();
            Map<String, Object> idLong = element_id.first().entrySet()
                    .stream()
                    .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
            ...
            collectionMap.insertOne(new Document(objectIdLong));
...
//        DBObject objectIdLong = collectionMap.findOne(new BasicDBObject("long_value", Long.toString(id)));
        FindIterable<Document> long_value = collectionMap.find(new BasicDBObject("long_value", Long.toString(id)));
//        Map<String,Object> idLong = (Map<String,Object>) objectIdLong.toMap();
        Map<String, Object> idLong = long_value.first()
                .entrySet()
                .stream()
                .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
...
//        DBObject objectIdLong = collectionMap.findOne(new BasicDBObject("element_id", ID));
        FindIterable<Document> element_id = collectionMap.find(new BasicDBObject("element_id", ID)).limit(1);
//        return objectIdLong != null;
        return element_id != null && element_id.first() != null;
    }
...
//            collection.insert(user);
    ...
    private boolean isUserItemInDB(String userID, String itemID) {
        ...
        return collection.find(query) != null && collection.find(query).first() != null;
    }
